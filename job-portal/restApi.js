...
//To apply job search
//For migrating all the data stored in Ms Sql DB to Elastic Search
app.put('/indexjob', function (req, res) {
    utility.printLog("Indexing Request Received", constants.LOG_LEVEL.INFO);
...
app.get('/search/', function (req, res) {
    ...
    let jobId = req.query.jobId;
    let online_intrvw_flag = req.query.online_intrvw_flag;
    if (!isAuthenticated(req)) {
        unAuthorisedResponse(res);
    } else if (!requiredSkills && !preferredSkills && !minYearsOfExperience && !candidateId && !transactionId && !companyId && !jobId && !online_intrvw_flag) {
        res.status(400).send({ "message": "Required Parameters not passed" })
    }
    else if (jobId) {
        jobService.search(jobId).then(results => {
            ...
            candidateService.search(requiredSkills, preferredSkills, minYearsOfExperience, offset, size, candidateId, transactionId, companyId, online_intrvw_flag).then(results => {
                let total = results.hits.total;
                let arr = [];
                for (let j in results.hits.hits) {
                    arr.push(results.hits.hits[j]._source);
